...
class AlertsTab(TabPane):
    """The existing alert triage view."""
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            with FilterPanel():
                yield Label("[b]Filtry[/b]")
                yield Label("Severity")
                yield Select(severity_options(), id="al-sev", value=SeverityBand.ALL.value)
                yield Label("Host")
                yield Select([("", "Wszystkie hosty")], id="al-host", value="", allow_blank=True)
                yield Label("MITRE tag")
                yield Select([("", "Wszystkie tagi")], id="al-mitre", value="", allow_blank=True)
                yield Label("Status triage")
                yield Select(triage_filter_options(), id="al-triage", value="all")
                yield Label("Szukaj")
                yield Input(placeholder="opis, reguła, notatka…", id="al-search")
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("Czas", "Lvl", "Host", "Reguła", "MITRE", "Status", "Opis")
        self.query_one("#al-detail", DetailPanel).mount(
            Static("[dim]Wybierz alert z tabeli[/dim]")
        )
        self.load_data()
    ...
    def load_data(self) -> None:
        try:
            ...
            self._refresh_table()
            self.app.set_status(
                f"Załadowano {len(self._all_alerts)} alertów z {self._alerts_path.name}"
            )
        except Exception as exc:
            self.app.set_status(f"Błąd ładowania alertów: {exc}")

    def _populate_selects(self) -> None:
        host_s = self.query_one("#al-host", Select)
        host_s.set_options([("", "Wszystkie hosty")] + [(h, h) for h in unique_hosts(self._all_alerts)])
        mitre_s = self.query_one("#al-mitre", Select)
        mitre_s.set_options([("", "Wszystkie tagi")] + [(t, t) for t in unique_mitre_tags(self._all_alerts)])
    ...
    def _refresh_table(self) -> None:
        ...
        active = " [aktywne]" if self._filters.is_active() else ""
        self.query_one("#al-summary", Static).update(
            f"\n[b]Wynik:[/b] {len(self._filtered_alerts)}/{len(self._all_alerts)}{active}"
        )
        self.app.set_status(
            f"Wyświetlono {len(self._filtered_alerts)} z {len(self._all_alerts)} alertów"
        )
    ...
    def _show_detail(self, alert_id: str) -> None:
        ...
        panel.mount(
            ...
            Static(f"[b]Reguła:[/b] {alert.rule_id}"),
            Static(f"[b]Czas:[/b] {alert.timestamp}"),
            Static(f"[b]MITRE:[/b] {alert.mitre_display}"),
            Static(
                f"[b]Taktyki:[/b] {', '.join(alert.mitre_tactics) or '—'}  "
                f"[b]Techniki:[/b] {', '.join(alert.mitre_techniques) or '—'}"
            ),
            Static(f"[b]Opis:[/b] {alert.description}"),
            Static(f"[{tri}][b]Status triage:[/b] {triage.status.label}[/{tri}]"),
            Label("Status triage"),
            Select(triage_action_options(), id="al-det-status", value=triage.status.value),
            Label("Analityk"),
            Input(value=triage.analyst, placeholder="np. jan.kowalski", id="al-det-analyst"),
            Label("Notatki analityka"),
            TextArea(triage.notes, id="al-det-notes"),
            Static(
                f"[dim]Ostatnia aktualizacja: {triage.updated_at or 'brak'}[/dim]",
                id="al-det-updated",
            ),
        )
    ...
    def save_triage(self) -> None:
        if not self._selected_id:
            self.app.set_status("Wybierz alert z tabeli")
            return
        try:
            ...
            self.query_one("#al-det-updated", Static).update(
                f"[dim]Ostatnia aktualizacja: {t.updated_at}[/dim]"
            )
            self._refresh_table()
            self.app.set_status("Zapisano triage")
        except Exception:
            self.app.set_status("Brak panelu szczegółów — wybierz alert")

    def clear_selection(self) -> None:
        self._selected_id = None
        panel = self.query_one("#al-detail", DetailPanel)
        panel.remove_children()
        panel.mount(Static("[dim]Wybierz alert z tabeli[/dim]"))
...
class DecoderLabTab(TabPane):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            with Vertical(id="lab-sidebar"):
                yield Label("[b]Import logów[/b]")
                yield Input(placeholder="ścieżka pliku .log/.txt/.jsonl/.json", id="lab-path")
                yield Button("Wczytaj plik", id="lab-load-file", variant="primary")
                yield Button("Wczytaj katalog", id="lab-load-dir", variant="default")
                yield Static("", id="lab-import-status")

                yield Label("[b]SSH (wazuh-logtest)[/b]")
                yield Input(placeholder="host IP/DNS", id="lab-ssh-host")
                yield Input(placeholder="user (domyślnie root)", id="lab-ssh-user")
                yield Input(placeholder="port (domyślnie 22)", id="lab-ssh-port")
                yield Input(placeholder="klucz ~/.ssh/id_rsa", id="lab-ssh-key")
                yield Button("Test SSH", id="lab-ssh-test", variant="default")
                yield Static("", id="lab-ssh-status")

                yield Button("Eksportuj XML", id="lab-export-xml", variant="success")
                yield Button("Eksportuj liblognorm", id="lab-export-rb", variant="success")

            with VerticalScroll(id="lab-center"):
                yield Static("[b]Klastry logów[/b] (wybierz wiersz)")
                yield DataTable(id="lab-cluster-table", zebra_stripes=True)
                yield Static("", id="lab-coverage")

                yield Label("Wazuh decoder XML [dim](kandydat)[/dim]")
                yield TextArea("", id="lab-gen-xml", language="xml")

                yield Label("liblognorm rulebase")
                yield TextArea("", id="lab-gen-rb")

                yield Label("Próbka z klastra")
                yield TextArea("", id="lab-sample-detail", read_only=True)

            with VerticalScroll(id="lab-right"):
                yield Static("[b]Podgląd predecodera[/b]")
                yield Static("", id="lab-predecoder-info")
                yield Static("[b]Wynik wazuh-logtest (SSH)[/b]")
                yield Button("Uruchom logtest (próbka)", id="lab-run-logtest", variant="warning")
                yield Button("Uruchom logtest (cały klaster)", id="lab-run-logtest-all", variant="default")
    ...
    def on_mount(self) -> None:
        ...
        table.cursor_type = "row"
        table.add_columns("ID", "Program", "Próbek", "Coverage", "Szablon")
    ...
    def _load_file(self) -> None:
        ...
        if not path_str:
            self._set_import_status("[red]Podaj ścieżkę do pliku[/red]")
            return
        p = Path(path_str)
        if not p.exists():
            self._set_import_status(f"[red]Nie znaleziono: {p}[/red]")
            return
    ...
    def _load_dir(self) -> None:
        path_str = self.query_one("#lab-path", Input).value.strip()
        if not path_str:
            self._set_import_status("[red]Podaj ścieżkę do katalogu[/red]")
            return
        p = Path(path_str)
        if not p.is_dir():
            self._set_import_status(f"[red]To nie jest katalog: {p}[/red]")
            return
    ...
    def _do_import(self, path: Path, is_dir: bool) -> None:
        try:
            self.app.call_from_thread(
                self._set_import_status, f"[yellow]Wczytywanie {path.name}…[/yellow]"
            )
            if is_dir:
            ...
            self.app.call_from_thread(
                self._set_import_status, f"[red]Błąd importu: {exc}[/red]"
            )
    ...
    def _apply_import_result(self, samples: list[LogSample], clusters: list[LogCluster]) -> None:
        ...
        self._set_import_status(
            f"[green]{len(samples)} próbek → {len(clusters)} klastrów[/green]"
        )
        self.app.set_status(f"Decoder Lab: {len(samples)} próbek, {len(clusters)} klastrów")
    ...
    def _show_cluster_detail(self, cluster: LogCluster) -> None:
        ...
        unmatched_preview = "\n".join(cov.unmatched_samples[:5])
        cov_markup = f"[b]Coverage:[/b] {cov.label}"
        if cov.unmatched_samples:
            cov_markup += f"  [dim](pierwsze niepasujące poniżej)[/dim]"
        self.query_one("#lab-coverage", Static).update(cov_markup)

        # Representative sample + predecoder
        ...
        if rep:
            ...
            if cov.unmatched_samples:
                predecoder_markup += f"\n\n[b]Niepasujące ({len(cov.unmatched_samples)}):[/b]\n"
                predecoder_markup += "\n".join(cov.unmatched_samples[:3])
        else:
            predecoder_markup = "[dim]brak próbek[/dim]"
    ...
    def _ssh_test(self) -> None:
        ...
        if not cfg.is_configured():
            self.query_one("#lab-ssh-status", Static).update("[red]Podaj adres hosta[/red]")
            return
        self.query_one("#lab-ssh-status", Static).update("[yellow]Łączenie…[/yellow]")
    ...
    def _run_ssh_test(self, cfg: WazuhSSHConfig) -> None:
        ...
        if r.error and "not found" not in r.error.lower():
            msg = f"[red]SSH błąd: {r.error}[/red]"
        else:
            msg = f"[green]SSH OK — wazuh-logtest odpowiada[/green]"
        self.app.call_from_thread(
    ...
    def _run_logtest_single(self) -> None:
        if not self._selected_cluster:
            self.app.set_status("Wybierz klaster z tabeli")
            return
        rep = self._selected_cluster.representative()
        if not rep:
            self.app.set_status("Klaster nie ma próbek")
            return
        cfg = self._read_ssh_cfg()
        if not cfg.is_configured():
            self.app.set_status("Podaj dane SSH w Decoder Lab")
            return
        self.query_one("#lab-logtest-out", TextArea).load_text("[yellow]Uruchamianie…[/yellow]")
    ...
    def _run_logtest_all(self) -> None:
        ...
        self.query_one("#lab-logtest-out", TextArea).load_text(
            f"[yellow]Wysyłam {len(samples)} próbek…[/yellow]"
        )
    ...
    def _show_logtest_results(self, results) -> None:
        if not results:
            self.query_one("#lab-logtest-out", TextArea).load_text("Brak wyników")
            return
        parts: list[str] = []
        ...
        out_text = "\n".join(parts)
        if len(results) > 10:
            out_text += f"\n… (skrócono, pokazano 10/{len(results)})"

        self.query_one("#lab-logtest-out", TextArea).load_text(out_text)
        total = len(results)
        self.query_one("#lab-logtest-summary", Static).update(
            f"[b]logtest coverage:[/b] {matched}/{total} z dekoderem"
        )
    ...
    def _export_xml(self) -> None:
        if not self._selected_cluster:
            self.app.set_status("Wybierz klaster przed eksportem")
            return
        ...
        self.app.set_status(f"Zapisano XML → {out}")
    ...
    def _export_rb(self) -> None:
        ...
        self.app.set_status(f"Zapisano rulebase → {out}")
...
class WazuhAlertViewer(App):
    ...
    BINDINGS = [
        ...
        Binding("1", "switch_tab('tab-alerts')", "Alerty"),
        Binding("2", "switch_tab('tab-lab')", "Decoder Lab"),
        Binding("r", "reload", "Odśwież"),
        Binding("s", "save_triage", "Zapisz triage"),
        Binding("/", "focus_search", "Szukaj"),
        Binding("escape", "clear_sel", "Wyczyść"),
    ]
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("Gotowy", id="status-bar")
        yield Footer()
